...
public class SelectView extends Composite {
	private Table tb_viewList;
	
	private CheckboxTableViewer ctv_viewlist;
	private UView currentView;
	 ...
	 * @param style
	 */
	public SelectView(Composite parent, int style) {
		super(parent, style);
		...
		btn_all.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (ctv_viewlist.getTable().getItemCount() == 0)
					return;
				if (ctv_viewlist.getCheckedElements().length == ctv_viewlist.getTable().getItemCount())
						ctv_viewlist.setAllChecked(false);
				else 
					ctv_viewlist.setAllChecked(true);

				if (ctv_viewlist.getCheckedElements().length > 0){
		...
		btn_inverse.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Object ao[] = ctv_viewlist.getCheckedElements();
				ctv_viewlist.setAllChecked(true);
				for (Object o:ao){
					ctv_viewlist.setChecked(o, false);
				}
				if (ctv_viewlist.getCheckedElements().length > 0){
					GVar.gWin.freshState();
				}
		...
		ctv_viewlist = CheckboxTableViewer.newCheckList(this, SWT.BORDER | SWT.FULL_SELECTION);
		ctv_viewlist.addCheckStateListener(new ICheckStateListener() {
...
//			}
//		});
		tb_viewList = ctv_viewlist.getTable();
		tb_viewList.setLinesVisible(true);
		tb_viewList.setHeaderVisible(true);
		tb_viewList.setToolTipText("\u9009\u62E9\u9700\u8981\u751F\u6210\u4EE3\u7801\u7684\u8868\u6A21\u578B");
		FormData fd_tb_viewList = new FormData();
		fd_tb_viewList.top = new FormAttachment(btn_inverse);
		fd_tb_viewList.bottom = new FormAttachment(100, -10);
		fd_tb_viewList.right = new FormAttachment(35, 10);
		fd_tb_viewList.left = new FormAttachment(0, 10);
		tb_viewList.setLayoutData(fd_tb_viewList);
		
		Group gb_viewDetail = new Group(this, SWT.NONE);
		FormData fd_gb_viewDetail = new FormData();
		fd_gb_viewDetail.top = new FormAttachment(tb_viewList, 0, SWT.TOP);
		fd_gb_viewDetail.left = new FormAttachment(tb_viewList);
		
		TableViewerColumn tvc_tcode = new TableViewerColumn(ctv_viewlist, SWT.NONE);
		TableColumn tc_tcode = tvc_tcode.getColumn();
		tc_tcode.setWidth(120);
		tc_tcode.setText("视图编码");
		
		TableViewerColumn tvc_tname = new TableViewerColumn(ctv_viewlist, SWT.NONE);
		TableColumn tc_tname = tvc_tname.getColumn();
		tc_tname.setWidth(150);
		tc_tname.setText("视图名");
		
		TableViewerColumn tvc_tremark = new TableViewerColumn(ctv_viewlist, SWT.NONE);
		TableColumn tc_tremark = tvc_tremark.getColumn();
		tc_tremark.setWidth(200);
		tc_tremark.setText("\u5907\u6CE8");
		gb_viewDetail.setLayout(new GridLayout(4, false));
		fd_gb_viewDetail.bottom = new FormAttachment(100, -5);
		fd_gb_viewDetail.right = new FormAttachment(100, -10);
		gb_viewDetail.setLayoutData(fd_gb_viewDetail);
		
		Label label = new Label(gb_viewDetail, SWT.NONE);
		label.setText("视图名：");
		
		tx_tname = new Text(gb_viewDetail, SWT.BORDER);
		...
		Label label1 = new Label(gb_viewDetail, SWT.NONE);
		label1.setText("\u7F16\u7801\uFF1A");
		
		tx_tcode = new Text(gb_viewDetail, SWT.BORDER);
		...
		Label label2 = new Label(gb_viewDetail, SWT.NONE);
		label2.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		label2.setText("\u5907\u6CE8\uFF1A");
		
		tx_remark = new Text(gb_viewDetail, SWT.BORDER);
		...
		tv_columns = new TableViewer(gb_viewDetail, SWT.BORDER | SWT.FULL_SELECTION);
		...
		fd_btn_toXml.right = new FormAttachment(gb_viewDetail, 0, SWT.LEFT);
		btn_toXml.setLayoutData(fd_btn_toXml);
		
		ctv_viewlist.setContentProvider(new IStructuredContentProvider(){
		...
		ctv_viewlist.setLabelProvider(new ITableLabelProvider(){
		       ...
		       public String getColumnText(Object element, int columnIndex) {
		            if (element instanceof UColumn){
		                ...
		                if(columnIndex == 0){
		                	...
		                	if (currentView == null)
		                		return null;
		                	return CAFun.GetColRemark(global.GVar.gModel,currentView,e);
		                }
		            }
		            return null;
		        }
		...
		tb_viewList.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent e) {
				TableItem[] items = tb_viewList.getSelection();
				if (items == null)
					return;
				if (items.length == 0){
					return;
				}
				currentView = GVar.gModel.getView(items[0].getText(0));
				if (currentView == null)
					return;
				UView t = currentView;
	...
	public void refresh(){
		if (global.GVar.gModel != null){
			...
			al.addAll(global.GVar.gModel.getViewList());
			ctv_viewlist.setInput(al);
		}
	}
	
	public ArrayList<UView> getSelectedViews(){
		ArrayList<UView> al = new ArrayList<UView>();
		for (Object o:ctv_viewlist.getCheckedElements()){
			UView t = (UView)o;
			al.add(t);
		}
		if (global.GVar.gModel != null)
			global.GVar.gModel.selectedViews = al;	
		return al;
	}
	public boolean isReady(){
		...
		if (GVar.gModel.selectedViews == null || GVar.gModel.selectedViews.size()==0)
			return false;
		if (ctv_viewlist.getCheckedElements().length == 0)
			return false;
		return true;
